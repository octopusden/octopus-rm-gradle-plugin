...
[WARNING]
====
*org.octopusden.octopus-release-management is deprecated and is not compatible with Gradle 9.*

Existing Gradle 8 projects may continue using this plugin. However, new projects and projects upgrading to Gradle 9 should use *org.octopusden.octopus-platform* instead.

For migration instructions, see link:{gradle9-support-link}[Gradle 9 Support] or https://github.com/octopusden/octopus-platform-gradle-plugin/blob/main/README.md[octopus-platform documentation].
====
...
=== Disabling automatic export of dependencies

By default, the `exportDependencies` task is automatically executed after a successful build when release dependencies are configured. To disable this automatic execution while still keeping the task available for manual invocation (e.g. via `gradle exportDependencies`), set the project property `autoExportDependencies` to `false`.

Via command line:

----
gradlew assemble -PautoExportDependencies=false
----

or in `gradle.properties`:
----
autoExportDependencies=false
----

*NOTE:*

- The `exportDependencies` task is always registered regardless of this setting. Setting `autoExportDependencies=false` only prevents its automatic execution after build; you can still run it manually.
- The legacy task name `exportDependenciesToTeamcity` is kept as a deprecated alias that simply depends on `exportDependencies`. It will emit a deprecation warning when invoked and may be removed in a future release. Migrate to `gradle exportDependencies`.

== Generate SBOM

The _'org.octopusden.octopus-release-management'_ Gradle plugin is able to autoconfigure creation of an aggregate of all direct and transitive runtime dependencies of a project and generate a valid CycloneDX SBOM. CycloneDX is a lightweight software bill of materials (SBOM) specification designed for use in application security contexts and supply chain component analysis. For enable generation provide '-Pcyclonedx.skip=false'. File will appear in 'build/generated-resources/sbom' directory.

The plugin uses _'org.cyclonedx.bom'_ plugin version https://github.com/CycloneDX/cyclonedx-gradle-plugin/tree/cyclonedx-gradle-plugin-1.7.4[1.7.4]. If one requires to change default configuration an added ":cyclonedxBom" task must be reconfigured in the project root. Example:
[source,groovy]
----
tasks.findByPath('cyclonedxBom')?.with {
    it.skipProjects = ['test-common'] //exclude 'test-common' module
    it.includeConfigs = ['runtimeClasspath', 'distribution'] //add 'distribution' gradle configuration
    //etc.
}
----
