...
NOTE: The `exportDependencies` task is always registered regardless of this setting. Setting `autoExportDependencies=false` only prevents its automatic execution after build; you can still run it manually.
