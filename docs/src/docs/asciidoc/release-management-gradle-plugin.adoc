...
By default, the `exportDependenciesToTeamcity` task is automatically executed after a successful build when release dependencies are configured. To disable this automatic execution while still keeping the task available for manual invocation (e.g. via `gradle exportDependenciesToTeamcity`), set the project property `autoExportDependencies` to `false`.
...
NOTE: The `exportDependenciesToTeamcity` task is always registered regardless of this setting. Setting `autoExportDependencies=false` only prevents its automatic execution after build; you can still run it manually.
